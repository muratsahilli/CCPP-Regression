...
for i in col_name_inputs:
    ...
    cross_cor = np.abs(np.corrcoef(dfscale[i], dfscale['elektrik enerjisi']))
    cross_correlation_array.append(cross_cor[0,1])
    
# make new dataframe with the half of the columns of dataframe which R2 values are highest 
half_data_cross=[]
list_for_colnames=[] 
for i in range(len(cross_correlation_array)//2):
    #take highest value's  index number
    imax = np.argmax( cross_correlation_array)
    #get data from original dataframe with index number
    max_values= dfscale[col_name[imax]].to_numpy()
    half_data_cross.append(max_values)
    #change highest value to lowest
    cross_correlation_array[imax] *= -1
    list_for_colnames.append(col_name[imax])
    
half_data_cross.append(dfscale['elektrik enerjisi'].to_numpy())
list_for_colnames.append('elektrik enerjisi')
df_half_data_cross =  pd.DataFrame(half_data_cross).T
df_half_data_cross.columns = list_for_colnames


# veri seti içindeki değişkenlerin dağılımlarının çizdirilmesi
df_half_data_cross.hist(bins=10,figsize=(16,9),grid=False);
...
corr=np.abs(df_half_data_cross.corr(method='pearson'))
plt.figure()

sns.heatmap(corr, annot = True)

features= df_half_data_cross.drop(columns=['elektrik enerjisi'])
elektrik = df_half_data_cross['elektrik enerjisi']
# Verileri çıkış değerine karşılık çizdirme
plt.figure(figsize=(20, 5))
for i, col in enumerate(features.columns):
    # 3 plots here hence 1, 3
    plt.subplot(1, len(features.columns), i+1)
    x = df_half_data_cross[col]
    y = elektrik
    plt.plot(x, y, 'o')
...
X = df_half_data_cross.iloc[:, :-1].values
Y = df_half_data_cross['elektrik enerjisi'].values
seed = 7
num_folds = 10
...
d = {'K-Fold-Cross-PrePro': ['mse-mean', 'r2-mean'], 'LR': lr_list, 'RIDGE': ridge_list, 'LASSO': lasso_list,
