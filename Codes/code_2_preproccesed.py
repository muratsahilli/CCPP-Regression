...
dfscale.hist(bins=10,figsize=(16,9),grid=False);
...
corr=np.abs(dfscale.corr(method='pearson'))
plt.figure()
sns.heatmap(corr, annot = True)

features = dfscale.drop('elektrik enerjisi', axis = 1)
elektrik = dfscale['elektrik enerjisi']
...
X = dfscale.iloc[:, :-1].values
Y = dfscale['elektrik enerjisi'].values
seed = 7
num_folds = 10
...
d = {'K-Fold-PrePro': ['mse-mean', 'r2-mean'], 'LR': lr_list, 'RIDGE': ridge_list, 'LASSO': lasso_list,
